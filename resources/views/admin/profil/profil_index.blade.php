				...
				<div class="tab-pane {{ ($errors->has('name') || $errors->has('noTelp') || $errors->has('email'))? 'active' : '' }}" id="settings">
					<form class="form-horizontal" method="POST" action="{{ route('admin.profil.update', ['profil' => 'update']) }}">
						...
						<div class="form-group">
							...
							<div class="col-sm-10">
								<input type="text" class="form-control" id="name" name="name" placeholder="Name" value="{{ Auth::guard('web')->user()->name }}" required>
						...
						<div class="form-group">
							...
							<div class="col-sm-10">
								<input type="email" class="form-control" id="email" name="email" placeholder="Email" value="{{ Auth::guard('web')->user()->email }}" required>
